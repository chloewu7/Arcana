...
**学习内容输入（更新）**：用户手动填写今日学了什么、学到哪一层、哪些尚未学习。笔记自动保存到本地当日草稿。先由 AI 整理已学牌、牌阵和知识点，用户可编辑知识点、取消牌与牌阵、确认是否包含逆位和象征，再生成作业。

出题只接收这次确认的范围，不依据浏览、掌握状态、到期复习或历史薄弱点扩展内容。生成结果检查卡牌白名单、未学牌名、逆位限制、题型与学习知识点；不通过时沿用 JSON 校验重试。仅基础知识也可出题，不要求指定牌面。每道题保存 `learningPoint`，必须对应一个确认的知识点。

学习笔记及确认范围随作业保存为 `studyNotes` / `studyScope`。支持重新填写内容生成新作业，旧作业和答案保留。每日入口显示当日最新一份作业。批改后的复习进度继续记录，但不会自动扩大新作业范围。

**题型**（AI 从中选择，混合搭配）：

| type | 说明 |
|---|---|
| `concept` | 已学基础概念，不要求指定牌面 |
...
- **出题**：只接收用户填写的学习内容和确认范围；题目要能暴露理解偏差而不是考记忆；不得引入未学内容；每题必须给 `rubric`；不要出选择题。
...
- `learningJournal` 按日期长期保存手动学习记录，同日补充保留，不随 90 天作业清理删除。
- 无需 AI 即可保存今日记录；跨日草稿、旧作业笔记自动归档，包含在导出备份中，兼容没有该字段的旧备份。
- 出题可主动选择往日记录，整理范围时仅合并今日与选中记录。
- 有今日新内容时，复习题数为 `max(1, floor(count / 3))`；仅复习时为总题数。返回 `learningSource` 标记 today / review，校验复习数量；作业保存 `reviewSources` 来源。
- 依照每张牌在对应笔记中明确学习的层次出题，不将一张牌的逆位学习泛化到其他牌。


## 13. 牌面学习状态完全手动

- 78 张牌默认均为未开始，打开详情、生成解读、作业批改不改变掌握状态。
- 详情页可手动选择未开始、学习中、已熟悉、已掌握，允许随时往回调整。
- 批改只更新复习次数和间隔，不升级或降级状态。
- 旧版本没有手动来源标记的状态迁移回未开始，保留解读、作业、复习次数；新的手动状态保存和导入后保持。
